<template>
  <div class="main">
    ...
    <div class="table">
      ...
      <input type='date' v-model="selectedVal" placeholder="ok"/>
      <table>
        <thead>
...
export default {
  ...
  created() {
    ...
    // get selected time
    let date = new Date();
    this.selectedVal = moment(date).format("YYYY-MM-DD");
  },
  watch: {
  ...
  methods: {
    ...
    getData: async function () {
      const url = "http://localhost:8123/predict_results";
      // POST request using axios with error handling
      let request_body = {
        "date": this.selectedVal
      }
      const headers = { 
